...
(function (global) {
  ...
  global.amazonUtils = {
    ...
    /**
     * 根据 JSON Schema 定义格式化 submitData
     * @param {Object} submitData - 原始提交数据
     * @param {Array} newProperties - 从 Schema 解析出的属性定义数组
     * @returns {Object} 格式化后的数据对象
     */
    formatSubmitDataBySchema: function (submitData, newProperties) {
      const result = {};
      const {
        marketplaceId: DEFAULT_MARKETPLACE_ID,
        languageTag: DEFAULT_LANGUAGE_TAG,
      ...
      // 遍历 newProperties 中的每个属性定义
      newProperties.forEach((propertyDef) => {
        // 获取属性名（newProperties 中每个对象只有一个键）
        const propertyName = Object.keys(propertyDef)[0];
        const schemaInfo = propertyDef[propertyName];

        // 检查 submitData 中是否有对应的数据
        if (submitData[propertyName]) {
          const originalData = submitData[propertyName];

          // 根据 Schema 定义格式化数据
          result[propertyName] = amazonUtils.formatPropertyBySchema(
            originalData,
            schemaInfo,
     ...
     * @returns {*} 格式化后的数据
     */
    formatPropertyBySchema: function (originalData, schemaInfo, marketplaceId, languageTag) {
      // 如果是数组类型
      if (schemaInfo.type === "array" && schemaInfo.items) {
        return amazonUtils.formatArrayProperty(originalData, schemaInfo.items, marketplaceId, languageTag);
      }

      // 如果是对象类型
      if (schemaInfo.type === "object" && schemaInfo.properties) {
        return amazonUtils.formatObjectProperty(originalData, schemaInfo.properties, marketplaceId, languageTag);
      }
    ...
    /**
     * 格式化数组类型属性
     * @param {Array} originalData - 原始数组数据
     * @param {Object} itemsSchema - items 的 Schema 定义
     * @param {string} marketplaceId - 市场ID
     * @param {string} languageTag - 语言标签
     * @returns {Array} 格式化后的数组
     */
    formatArrayProperty: function (originalData, itemsSchema, marketplaceId, languageTag) {
      ...
      if (itemsSchema.type === "object" && itemsSchema.properties) {
        ...
        // 遍历 items.properties 中定义的属性
        Object.keys(itemsSchema.properties).forEach((propName) => {
          const propSchema = itemsSchema.properties[propName];

          if (firstItem[propName] !== undefined) {
            // 递归处理嵌套属性
            formattedItem[propName] = amazonUtils.formatPropertyBySchema(
              firstItem[propName],
              propSchema,
              marketplaceId,
              languageTag
      ...
      // 如果 items 是基本类型，保持原有结构但确保有 marketplace_id 和 language_tag
      return [{
        marketplace_id: marketplaceId,
        language_tag: languageTag,
        value: firstItem.value || firstItem
      }];
    },

    /**
     * 格式化对象类型属性
     * @param {Object} originalData - 原始对象数据
     * @param {Object} propertiesSchema - properties 的 Schema 定义
     * @param {string} marketplaceId - 市场ID
     * @param {string} languageTag - 语言标签
     * @returns {Object} 格式化后的对象
     */
    formatObjectProperty: function (originalData, propertiesSchema, marketplaceId, languageTag) {
      const result = {};

      // 遍历 Schema 中定义的属性
      Object.keys(propertiesSchema).forEach((propName) => {
        const propSchema = propertiesSchema[propName];

        if (originalData[propName] !== undefined) {
          result[propName] = amazonUtils.formatPropertyBySchema(
            originalData[propName],
            propSchema,
            marketplaceId,
            languageTag
          );
  ...
  global.amazonFormatSubmitDataBySchema = global.amazonUtils.formatSubmitDataBySchema;
