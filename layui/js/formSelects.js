...
(function(layui, window, factory) {
    ...
    let v = '4.0.0.0910',
        ...
        FormSelects = function(options) {
            this.config = {
                ...
                filter: (id, inputVal, val, isDisabled) => {
                    ...
                    if (newInputVal.indexOf(',') > 0) {
                        // 多个精确
                        let inputValArr = newInputVal.trim().split(',').map(val => val.trim());
                        return !inputValArr.includes(val.name);
                    } else {
                        // 单个模糊
                        return val.name.toLowerCase().indexOf(newInputVal.toLowerCase().trim()) === -1;
                    }
            ...
            $.extend(this.config, options, {
                ...
                btns: options.radio ? [quickBtns[1]] :options.allPeople ? [quickBtns[0], quickBtns[1], quickBtns[4],quickBtns[2]] : [quickBtns[0], quickBtns[1], quickBtns[2]],
        ...
        $((target ? target : `select[${NAME}]`)).each((index, select) => {
            ...
            let reElem =
                $(`<div class="${FORM_SELECT}" ${SKIN}="${fs.config.skin}">
					...
					<dl xid="${id}" class="${DL} ${fs.config.radio ? RADIO:''}" style="${isAllpelple?'min-width:380px;':''}">${dinfo}</dl>
				</div>`);
    ...
    Common.prototype.createDD = function(id, item, clz) {
        ...
        if(isAllpelple){
            return `<dd employee="${employee}"  lay-value="${item.value}" class="${item.disabled ? DISABLED : ''} ${clz ? clz : ''} ${employeeDisplay}" ${attr}>
    ...
    Common.prototype.renderSelect = function(id, tips, select) {
        ...
        if (data[id].config.btns.length) {
            setTimeout(() => {
                ...
                if(isAllpelple){
                    dl.find(`.${CZ_GROUP}`).css('max-width', `${dl.prev().width() + 100}px`);
                }else{
    ...
    Common.prototype.selectAll = function(id, isOn, skipDis) {
        ...
        // 全选不操作disN隐藏的元素
        dl.find(`dd[lay-value]:not(.${FORM_SELECT_TIPS}):not(.${THIS}):not('.disN'):not(.${DD_HIDE})${skipDis ? ':not(.'+DISABLED+')' :''}`).each((index, item) => {
            item = $(item);
    ...
    Common.prototype.allpeopleFn = function(id, isOn, skipDis) {
        ...
        let dd=dl.find(`dd[lay-value]:not(.${FORM_SELECT_TIPS}):not(.${DD_HIDE})${skipDis ? ':not(.'+DISABLED+')' :''}`)
        ...
        // let ddArr=[]
            if(peopleDD){
                if(peopleDD.hasClass('allPeopleStyle')){
                    ...
                    dd.filter('.xm-select-this[employee=employee]').each((i,item)=>{//点击全部人员按钮时，对离职人员的操作（从input框去掉离职人员的名字名字）
                        let case_name=$(item).find('span').attr('name');
                        let ddObj={name:case_name,value:Number($(item).attr('lay-value')),XM_PID_VALUE:$(item).attr('tree-id'),disabled:false,innerHTML:case_name};
                    ...
                    dd.filter('.xm-select-this[employee=employee]').each((i,item)=>{//点击全部人员按钮时，对离职人员的操作（把离职人员的名字添加到input框）
                        let case_name=$(item).find('span').attr('name');
                        let ddObj={name:case_name,value:Number($(item).attr('lay-value')),XM_PID_VALUE:$(item).attr('tree-id'),disabled:false,innerHTML:case_name};
